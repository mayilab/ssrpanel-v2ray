...
public class V2rayDao {
    ...
    private static final String GET_ALL_USER = "SELECT u.id, u.vmess_id vmessId FROM `user` u LEFT JOIN user_label ul ON ul.user_id = u.id WHERE ul.label_id = "+ConfigUtil.getInteger("label.id")+"AND u.`enable` = 1 AND u.u + u.d < u.transfer_enable";
